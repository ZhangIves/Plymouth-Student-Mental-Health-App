...
import com.loong.x.chat.mapper.GroupMsgMapper;
...
public class GroupMsgServiceImpl extends ServiceImpl<GroupMsgMapper, GroupMsg> implements GroupMsgService {
    ...
    @Override
    public IPage<GroupMsgVo> page(GroupMsgVo groupMsgVo, Integer current, Integer size,Long userId) {
        ...
        // 查询未读数量
        LambdaQueryWrapper<MsgUnreadRecord> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(MsgUnreadRecord::getUserId, userId).eq(MsgUnreadRecord::getTargetId, groupMsgVo.getGroupId());
        MsgUnreadRecord msgUnreadRecord = msgUnreadRecordService.getOne(queryWrapper);
