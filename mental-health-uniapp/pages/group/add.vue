...
<!--          <u-upload
		  ...
		  <view @click="chooseImage">
			  <image v-if="avatar" :src="avatar"
		            mode="aspectFit" style="width: 180rpx;height: 180rpx;"></image>
			  <view v-else style="background-color: #cccac7;padding: 100rpx;">
				  <image src="../../static/img/photo.png" style="width: 80rpx;height: 80rpx;"></image>
			  </view>
		  </view>
        </u-form-item>
	  ...
	  UploadApi.image(file)
	  	.then(files => {
	  		if (files && files.length > 0) {
	  			app.model.group.avatar = files[0].domain + files[0].fileName;
	  			app.flagA = true
	  			if(result.code){
	  				app.$tips.success("上传成功")
	  			}
	  		}
	  		resolve(files)
	  	})
